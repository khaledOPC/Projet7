...
def test_clean_search():
    ...
    assert clean_search("@@-* je veux !$paris") == "paris"
    assert (
        clean_search("Bonjou'r à une information sur paris")
        == "Bonjour information paris"
    )
...
def test_search_google(mock_requests):
    ...
    mock_response.json.return_value = {
        "candidates": [
            {
                "formatted_address": "27 rue test_adresse",
                "geometry": {"location": {"lat": 30, "lng": 20}},
            }
        ]
    }
...
def test_search_wikipedia_1(mock_requests):
        ...
        "query": {
            "pages": {
                "11498241": {
                    "pageid": 11498241,
                    "ns": 0,
                    "title": "Place Sébastopol",
                    "index": -1,
                    "extract": "Bonjour lille",
                }
            }
        }
    }
    ...
    assert (
        result
        == "Oups !.. Je n'ai aucune information à ce sujet essayez autres choses !"	# noqa
    )
